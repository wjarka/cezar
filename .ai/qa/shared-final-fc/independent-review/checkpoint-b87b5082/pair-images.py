...
main=[p for p in pairs if p['name']==p['reference'] or p['name'] in ['import-desktop-light','import-desktop-dark','import-mobile-light','import-mobile-dark']]
(out/'comparisons').mkdir(exist_ok=True)
for p in main:
    actual=Image.open(out/p['actual']).convert('RGB')
    reference=Image.open(Path('.ai/design-reference/iteration-3/design')/f"{p['reference']}.png").convert('RGB')
