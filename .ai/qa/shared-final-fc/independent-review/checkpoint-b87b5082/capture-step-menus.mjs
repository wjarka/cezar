...
const pairs=JSON.parse(readFileSync(out+'/pairs.json')).filter(p=>!p.name.startsWith('step-menu-'));
...
for(const id of ['C2sfEo','aa0TQ','s0JzCL','Ae0n6']){
 ...
 await page.locator('[data-slot="wb-step-actions"]').first().click(); await page.getByRole('menu').waitFor(); await snap('step-menu-'+id,id,viewport,theme); await page.keyboard.press('Escape');
}
