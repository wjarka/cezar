...
const qa=resolve(source,'.ai/qa/shell-current193');
const repo='/tmp/cezar-shell-a439/iac';
...
for(const name of ['website','earwitness']) {
 const root='/tmp/cezar-shell-a439/'+name; mkdirSync(root+'/.ai/cezar',{recursive:true});
...
mkdirSync(qa+'/cez-home',{recursive:true});writeFileSync(qa+'/cez-home/config.json',JSON.stringify({projects:['iac','website','earwitness'].map((name,i)=>({id:name,name,root:'/tmp/cezar-shell-a439/'+name,source:'local',addedAt:'2026-09-01T00:00:00.000Z',lastOpenedAt:`2026-09-${12-i}T00:00:00.000Z`}))}));
