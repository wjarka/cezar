...
def record(name,cmd,code,start):
 r=dict(command=' '.join(cmd),exitCode=code,seconds=round(time.time()-start,1));results.append(r);(root/'verified-final-gate-results.json').write_text(json.dumps(results,indent=2)+'\n');print(name,r,flush=True)
def run(name,cmd):
 start=time.time()
 with (root/('verified-final-'+name+'.log')).open('w') as f:code=subprocess.call(cmd,env=env,stdout=f,stderr=subprocess.STDOUT)
...
browserLog=(root/'verified-final-browser.log').open('w');browserStart=time.time()
...
browserCode=browser.wait();browserLog.close();(root/'verified-final-browser-result.json').write_text(json.dumps(dict(command=' '.join(browserCmd),exitCode=browserCode,seconds=round(time.time()-browserStart,1)),indent=2)+'\n');print('browser',browserCode,flush=True)
