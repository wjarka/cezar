...
const source=process.cwd(),qa=resolve('.ai/qa/final-integrated/checkpoint-settings'),runtime=resolve('.ai/qa/runtime-verified');
...
for(const frame of frames.filter(f=>/^(S03|S12)\./.test(f.name))){
...
writeFileSync(qa+'/pairs.json',JSON.stringify({designSourceSha256:manifest.penFileSha256,sourceCommit,sourceSha256,indexSha256,servedIndexSha256,pairs},null,2));
if(pairs.length!==8)throw Error('Incomplete capture');
console.log('Captured eight checkpoint Settings pairs');
