...
served=urllib.request.urlopen('http://127.0.0.1:44659/').read()
...
'14':'Task header and toolbar are baseline shared implementations. Preserves full task-attributed diff, hierarchical tree and all Git actions. Source abbreviates diff, relocates toolbar below it, and shows four files while this fixture carries three. Mobile retains automatic unified/wrapped behavior.',
...
'R2':'Actual persisted list widths 280/360/520px captured in both themes. Same GitHub document/control differences as family 9; source and runtime share CSS viewport while their surrounding shared shell implementations differ.'}
common='Shared sidebar, task header, global colors and shared Diff implementation remain the worker baseline; parent integration is required for whole-screen fidelity. Poppins is observed in the browser. Local 23 icon outlines are exact geometry matches to corrected source; unmatched shared/utility icons remain baseline.'
...
m.update(sourceSha256=sourceHash,buildCommit=commit,webDistSha256=buildHash,reviewedAt=datetime.datetime.now(datetime.timezone.utc).isoformat(),status='scoped implementation reviewed; integrated fidelity pending')
