...
for theme in ['light','dark']:
 for width,height in [(1440,1300),(402,2001)]:
  ...
  browser('open','http://127.0.0.1:44659/github/prs/218/changes');browser('wait','[data-slot="gh-merge-box"]')
  ...
  for state,payload in [('text',dict(content='# Readme\n\nPopulated file preview.\n',binary=False,tooLarge=False)),('binary',dict(binary=True,tooLarge=False)),('too-large',dict(binary=False,tooLarge=True))]:
   ...
   route('/runs/fixture-session/files?path=README.md',dict(type='file',path='README.md',size=4096,**payload))
   browser('open','http://127.0.0.1:44659/tasks/fixture-session/files');browser('wait','[data-slot="files-tree"]');browser('click','[data-slot="files-file"][data-path="README.md"]')
