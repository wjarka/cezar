...
repo['info'].update(root='/fixture/cezar',branch='main',remote='hearsay-tools/cezarion')
...
m += [dict(path='/repo/changes*',body=changes),dict(path='/runs/10000000-0000-4000-8000-000000000000/changes*',body=changes),dict(path='/runs/10000000-0000-4000-8000-000000000000/commits',body={'commits':[dict(sha=x['hash'],subject=x['subject'],author=x['author'],when=x['when']) for x in repo['log'][:1]]})]
...
m += [dict(path='/runs/10000000-0000-4000-8000-000000000000/files?path=',body=dict(type='dir',path='',entries=entries))]
...
subprocess.run([str(out/'browser.sh'),'network','unroute'],check=True,stdout=subprocess.DEVNULL)
for mock in m:
