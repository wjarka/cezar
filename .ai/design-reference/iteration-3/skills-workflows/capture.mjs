...
const root=process.cwd(),out=resolve(root,'.ai/design-reference/iteration-3/skills-workflows');
...
const pairs=process.env.CEZ_QA_DIALOGS_ONLY?JSON.parse(readFileSync(out+'/pairs.json')).filter(p=>ids.includes(p.name)):[];
...
for(const id of (process.env.CEZ_QA_DIALOGS_ONLY ? [] : ids)){
 ...
 await page.goto('http://127.0.0.1:44633/p/fixture-repo/'+(screen==='workflows'?'workflows':'skills'+(screen==='bookmarklets'?'?skill=__bm':screen==='manage'?'?skill=__import':screen==='detail'?'?skill=award-level-visual-design':'')));
...
for(const theme of (process.env.CEZ_QA_DEBUG?['light']:['light','dark']))for(const mobile of (process.env.CEZ_QA_DEBUG?[true]:[false,true])){
 ...
 await page.goto('http://127.0.0.1:44633/p/fixture-repo/workflows/fix-and-verify');await page.locator('[data-slot="wb-name"]').waitFor();
 const tag=(mobile?'mobile':'desktop')+'-'+theme,ref=theme==='light'?'lRC5W':'KwkJs';
 if(mobile)await page.setViewportSize({width:402,height:420});
 await page.locator('[data-slot="wb-import"]').click();await page.locator('[data-slot="wb-import-text"]').fill('name: fix-and-verify\nskills:\n  - test-conventions\n  - commit-style');
 await snap('import-'+tag,mobile?(theme==='light'?'djknR':'MF4eF'):ref,mobile?{width:402,height:420}:viewport,theme,'[data-slot="wb-import-panel"]');
 if(process.env.CEZ_QA_DEBUG)break;
 await page.locator('[data-slot="wb-import-run"]').click();await page.locator('[data-slot="wb-import-error"]').waitFor();await snap('import-error-'+tag,mobile?(theme==='light'?'djknR':'MF4eF'):ref,mobile?{width:402,height:420}:viewport,theme,'[data-slot="wb-import-panel"]');
 await page.locator('[data-slot="wb-import-cancel"]').click();if(mobile)await page.setViewportSize(viewport);
...
for(const theme of ['light','dark']){
 ...
 for(const state of ['current','available','updating','error']){
  screen='manage';updateStatus=state;await page.goto('http://127.0.0.1:44633/p/fixture-repo/skills?skill=__import');await page.locator('[data-slot="skills-update-card"]').waitFor();await snap('manage-'+state+'-'+theme,theme==='light'?'MB5Yx':'IbcKi',viewport,theme,'[data-slot="skills-import-panel"]');
 }
 ...
 for(const err of ['skills-error','manage-error']){
  screen=err;await page.goto('http://127.0.0.1:44633/p/fixture-repo/skills'+(err==='manage-error'?'?skill=__import':''));await page.getByText(err==='skills-error'?'Could not load skills':'Could not load importable skills',{exact:true}).waitFor();await snap(err+'-'+theme,theme==='light'?'MB5Yx':'IbcKi',viewport,theme);
 }
}
